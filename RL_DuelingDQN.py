...
class DQN:
    ...
    def build_dueling_layers(self, state, c_names, w_initializer, b_initializer):
        ...
        with tf.variable_scope('Value'):
            w2 = tf.get_variable('w2', [self.n_l1, 1], initializer=w_initializer, collections=c_names)
    ...
    def build_net(self):
        ...
        with tf.variable_scope('eval_net'):
            ...
            self.q_eval = self.build_dueling_layers(self.states, c_names, w_initializer, b_initializer)

        # ------------------ build target_net ------------------
        self.next_states = tf.placeholder(tf.float32, shape=[None, self.n_state])
        with tf.variable_scope('target_net'):
            c_names = ["target_net_params", tf.GraphKeys.GLOBAL_VARIABLES]
            self.q_next = self.build_dueling_layers(self.next_states, c_names, w_initializer, b_initializer)
